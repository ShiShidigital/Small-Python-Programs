In this Git Repository I want to collect some 
small Python Programs,
that I wrote and are helpful to me.

### List of Programs 
- Decision Helper: Helps to decide about things. Command line program.
- Mastodon Public Timeline Reader: You can choose server 
  and number of posts. Prints to command line.
- Python Variable Checker
- Find most used word in a Text 
- Get ASCII number from a letter 
- Get text data from clipboard 
- Get Header info from webpage
- Get all links from a webpage

